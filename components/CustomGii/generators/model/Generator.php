...
use app\components\CustomGii\CodeFile;
 ...
 * @property-read string[] $schemaNames
 * @property-read \yii\db\Connection $dbConnection
 * @property-read array $customRelationWithPostgresql
 * @property-read array $customRelationWithMysql
 * @property-read null|string $dbDriverName
 */
class Generator extends \app\components\CustomGii\Generator
{
    const RELATIONS_NONE = 'none';
    const RELATIONS_ALL = 'all';
    const RELATIONS_ALL_INVERSE = 'all-inverse';
    const RELATIONS_ALL_CUSTOM = 'all-custom';
    ...
    public $generateRelations = self::RELATIONS_ALL_CUSTOM;
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            [['generateRelations'], 'in', 'range' => [self::RELATIONS_NONE, self::RELATIONS_ALL_CUSTOM, self::RELATIONS_ALL, self::RELATIONS_ALL_INVERSE]],
    ...
    public function hints()
    {
        return array_merge(parent::hints(), [
            ...
            'useTablePrefix' => 'This indicates whether the table name returned by the generated ActiveRecord class
                should consider the <code>tablePrefix</code> settings of the DB connection. For example, if the
    ...
    {
        $files = [];
        $relations = $this->generateRelations == self::RELATIONS_ALL_CUSTOM ? $this->generateCustomRelations() : $this->generateRelations();
        $db = $this->getDbConnection();
     ...
     * @return array the generated relation declarations
     */
    protected function generateCustomRelations()
    {
        if ($this->generateRelations === self::RELATIONS_NONE) {
            return [];
        }
        ...
        if($db->driverName == 'mysql'){
            $relations = $this->getCustomRelationWithMysql($db);
        }
        if($db->driverName == 'pgsql'){
            $relations = $this->getCustomRelationWithPostgresql($db);
        }
        return $relations;
    }
    protected function getCustomRelationWithMysql($db){
        $dbName = $this->getDsnAttribute('dbname', $db->dsn);
        $tableName = $this->tableName;
        $relations = [];
        $schemaNames = Yii::$app->db->createCommand("SELECT ke.REFERENCED_TABLE_SCHEMA parentSchema,
                                                        ...
                                                        WHERE ke.referenced_table_name IS NOT NULL
                                                          AND (ke.referenced_table_name = '{$tableName}' OR ke.table_name = '{$tableName}') AND ke.TABLE_SCHEMA = '{$dbName}' ## Find Foreign Keys linked to this Primary Key
                                                        ORDER BY
    ...
    protected function getCustomRelationWithPostgresql($db){
        $dbName = $this->getDsnAttribute('dbname', $db->dsn);
        $tableName = $this->tableName;
        $relations = [];
        $schemaNamesHasOne = Yii::$app->db->createCommand("select kcu.table_name as childtable,
                                                               rel_tco.table_name as parenttable,
                                                               string_agg(kcu.column_name, ', ') as childcolumnname,
                                                                'id' as parentcolumnname,
                                                               kcu.constraint_name
                                                        ...
                                                        where tco.constraint_type = 'FOREIGN KEY' AND kcu.table_catalog = '{$dbName}' AND kcu.table_name = '{$tableName}'
        ...
        if(!empty($schemaNamesHasOne)){
            foreach ($schemaNamesHasOne as $schemaName) {
                // Add relation for the referenced table
                $hasMany = $tableName == $schemaName['parenttable'];
                $link = $hasMany ? '['.$schemaName['childcolumnname'].' => '.$schemaName['parentcolumnname'].']' : '['.$schemaName['parentcolumnname'].' => '.$schemaName['childcolumnname'].']';
                $className = $hasMany ? Inflector::id2camel($schemaName['childtable'],'_') : Inflector::id2camel($schemaName['parenttable'],'_');
        ...
        $schemaNamesHasMany = Yii::$app->db->createCommand("select kcu.table_name as childtable,
                                                               rel_tco.table_name as parenttable,
                                                               string_agg(kcu.column_name, ', ') as childcolumnname,
                                                                'id' as parentcolumnname,
                                                               kcu.constraint_name
                                                        from information_schema.table_constraints tco
                                                                 ...
                                                                 join information_schema.table_constraints rel_tco
                                                                      on rco.unique_constraint_schema = rel_tco.constraint_schema
                                                                          and rco.unique_constraint_name = rel_tco.constraint_name
                                                        where tco.constraint_type = 'FOREIGN KEY' AND kcu.table_catalog = '{$dbName}' AND rel_tco.table_name = '{$tableName}'
                                                        group by kcu.table_schema,
                                                                 kcu.table_name,
                                                                 rel_tco.table_name,
                                                                 rel_tco.table_schema,
        ...
        if(!empty($schemaNamesHasMany)){
            foreach ($schemaNamesHasMany as $schemaName) {
                // Add relation for the referenced table
                $hasMany = $tableName == $schemaName['parenttable'];
                $link = $hasMany ? '['.$schemaName['childcolumnname'].' => '.$schemaName['parentcolumnname'].']' : '['.$schemaName['parentcolumnname'].' => '.$schemaName['childcolumnname'].']';
                $className = $hasMany ? Inflector::id2camel($schemaName['childtable'],'_') : Inflector::id2camel($schemaName['parenttable'],'_');
